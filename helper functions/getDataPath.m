...
%  fd = GETDATAPATH(cfg) Searches for specified promoted subject folders in the
...
if ~isempty(cfg.rats)
    for iRat = 1:length(cfg.rats)
        ...
        for iFD = 1:length(temp_fd)
            ...
            [~, temp_fn] = fileparts(this_pwd);
            this_fd_date = cat(1,this_fd_date,datetime(temp_fn(6:15))); % assumes %Sxxx-YYYY-MM-DD[postfix] dirname format
