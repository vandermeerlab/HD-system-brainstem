function getSlowPhaseTC(cfg_in, sd, iCell)
% JJS. 2024-03-12.
...
% myCell.t{1}(keep{1}==1)=NaN;   % I forget what this line was supposed to do
...
tc_outR = TuningCurves(cfg_tc, myCell, AHVr);
...
cfg_Q.tvec_edges = AHVr.tvec(1) : cfg_Q.dt : AHV_tsd.tvec(end);
...
tc_velR = TuningCurves(cfg_V, myCell, diffHr);
...
cfg_tc.minOcc = 100;  % remember that Occ is measured in samples (usually 5ms per sample), not in seconds
% tc_out = TuningCurves(cfg_tc, myCell, diffH);
tc_outR = TuningCurves(cfg_tc, myCell, orientationR);
if cfg_out.smooth
    plot(tc_outR.binCenters, smoothdata(tc_outR.tc), 'LineWidth', cfg_out.LineWidth, 'Color', 'k');
else
    plot(tc_outR.binCenters, tc_outR.tc, 'LineWidth', 3, 'Color', 'k');
...
% tc_out = TuningCurves(cfg_tc, myCell, diffH);
tc_outR = TuningCurves(cfg_tc, myCell, tsdHr);
if cfg_out.smooth
    plot(tc_outR.binCenters, smoothdata(tc_outR.tc), 'LineWidth', cfg_out.LineWidth, 'Color', 'k');
else
    plot(tc_outR.binCenters, tc_outR.tc, 'LineWidth', 3, 'Color', 'k');
...
[outputS, ~, ~, outputIT, cfg_spikes] = SpikePETH_either([], myCellr, combinedSaccadesToUse);
set(gca, 'FontSize', cfg_out.FontSize)
