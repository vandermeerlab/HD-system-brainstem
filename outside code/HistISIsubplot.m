function [H, binsUsed] = HistISIsubplot(T, varargin)
...
if nargout == 0 || ~isempty(axesHandle)  % no output args, plot it
    ...
    %     end
    %     plot(axesHandle, binsUsed(1:end-1), H, '-', 'color', myColor); hold on
    semilogx(binsUsed(1:end-1), H, '-', 'color', myColor); hold on
    c = axis;
    plot([0.001 0.001], [c(3) c(4)], 'r-', ...
        [0.002 0.002], [c(3) c(4)], 'g-');
...
%         xlabel('ISI (s).');
    ...
    axis([10^minLogISI 10^maxLogISI 0 max(H)])
    set(gca, 'YTick', max(H));
end
