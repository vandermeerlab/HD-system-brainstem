function [neuronID] = plotAHV_tc_CW_CCW(X, Z, tfilelist, varargin)
% 2024-11-26. JJS.
...
for iNeuron = startNeuron : endNeuron
    ...
    CCWline = X.bCCW(iNeuron)*X.cfg_AHV.binSize*(1:length(X.CCWbins)) + X.CCWyInt(iNeuron);   % y = mx + b
    plot(X.CCWbins, CCWline)
    
    CWline = X.bCW(iNeuron)*X.cfg_AHV.binSize*(1:length(X.CWbins)) + X.CWyInt(iNeuron);  %
    ...
    if doSave
        disp('saving figure')
        cd('D:\Jeff\U01\analysis\AHV selectivity JG definition\binned AHV TCs with slopes\TC plots w Captions')
