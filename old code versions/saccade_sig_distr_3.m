...
w = [];
w.pt95 = percent_t >=.95; w.spt95 = sum(w.pt95);
w.pn95 = percent_n >=.95; w.spn95 = sum(w.pn95);

w.t_percent_95 = w.spt95/numCells;
w.n_percent_95 = w.spn95/numCells;

w.pt99 = percent_t >=.99; w.spt99 = sum(w.pt99);
w.pn99 = percent_n >=.99; w.spn99 = sum(w.pn99);

w.t_percent_99 = w.spt99/numCells;
w.n_percent_99 = w.spn99/numCells;

w.f95_n = find(w.pn95); 
w.f95_t = find(w.pt95); 

w.intersect = intersect(f95_n, f95_t);
w.numSig_both = length(w.intersect); 
w.percent_sig_both = w.numSig_both/numCells;

C = {'spn95', 'spt95', 'spn99', 'spt99', 'n_percent_95', 't_percent_95', 'n_percent_99', 't_percent_99', 'f95_n', 'f95_t', ...
    'numSig_both', 'percent_sig_both', 'intersect', 'pn95', 'pt95', 'pn99', 'pt99'};
