function [X] = findAHVslopesFromTuningCurvesSavePlots(savedestination, varargin)
...
%             str = sprintf('%.2f', strcat('rsq =', num2str(rsqPos(cellCounter))));     
%             h = get(gca, 'XLim');
%             h2 = get(gca, 'YLim');

            text(50, 45, strcat('rsq =', num2str(rsqPos(cellCounter))))
            text(50, 25, strcat('p =', num2str(pPos(cellCounter))))
            ...
            xlabel('AHV (deg/sec)', 'FontSize', FontSize)
            ylabel('Firing Rate (Hz)', 'FontSize', FontSize)
            text(-100, 45, strcat('rsq =', num2str(rsqNeg(cellCounter))))
            text(-100, 25, strcat('p =', num2str(pNeg(cellCounter))))
            set(gca, 'FontSize', 16)
            c = axis; 
            ymax = max([c(4) 50]);
            axis([-200 200 0 ymax])
            hold on
            plot(tc_out.usr.binCenters, smoothdata(tc_out.tc(iCell,:)), 'LineWidth', 5)
            %             subplot(1,2,2)
            ...
            if doSave == 1
                baseFileName = S.label{iCell};            
                if strcmp(figType, 'fig') == 1
