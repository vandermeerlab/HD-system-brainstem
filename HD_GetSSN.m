function [OUT] = HD_GetSSN(scope)
% 2016-08-01. JJS. Adapted for plus maze task. 
%Simple function to grab the Rat SSN:  R###-YYYY-MM-DD
%2011-05-10 AndyP Added scope.

if nargin==0
	D = cd;
	strRat = regexp(D, 'M\d\d\d-\d\d\d\d-\d\d-\d\d', 'once');
	if isempty(strRat);
...
switch scope
	case 'MultipleSessions'
		...
		skip = 0;
		for iC=1:length(D);
			IsSession = regexp(D(iC,1).name, 'M\d\d\d-\d\d\d\d-\d\d-\d\d');
	...
	case 'SingleSession'
		...
		if nargin~=0
			D = cd;
			strRat = regexp(D, 'M\d\d\d-\d\d\d\d-\d\d-\d\d');
		end
		SSNstr = D(strRat:end);
		if ~isempty(SSNstr)
			OUT = SSNstr;
